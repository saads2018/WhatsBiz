...
namespace WASender
{
    public partial class GeneralSettings : MyMaterialPopOp
    {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                File.WriteAllText(fileSaves + "\\" + "Groups.json", "");
                File.WriteAllText(fileSaves + "\\" + "GroupSender.json", "");
                File.WriteAllText(fileSaves + "\\" + "IndividualContacts.json", "");
                File.WriteAllText(fileSaves + "\\" + "SingleSender.json", "");
                File.WriteAllText(fileSaves + "\\" + "GroupLinks.json", "");

                DirectoryInfo directoryInfo = new DirectoryInfo(fileSaves);
                foreach (FileInfo file in directoryInfo.GetFiles())
                {
                    if(file.Exists&&file.Extension==".bak")
                    file.Delete();
                }
                MaterialSnackBar SnackBarMessage = new MaterialSnackBar("Done 👍👍👍👍", Strings.OK, true);
                SnackBarMessage.Show(this);

                navPage.automaticMessagingNav1.chooseContact1.goToLists();
                navPage.automaticMessagingNav1.chooseContact1.label1.Text = "Contact List";
                navPage.automaticMessagingNav1.chooseContact1.label1.Location = new Point(navPage.automaticMessagingNav1.chooseContact1.label1.Location.X + 30, navPage.automaticMessagingNav1.chooseContact1.label1.Location.Y);
                navPage.automaticMessagingNav1.chooseGroupContacts1.View(0, "0");
                navPage.automaticMessagingNav1.chooseGroupContacts1.label1.Text = "Group List";
                navPage.automaticMessagingNav1.chooseGroupContacts1.label1.Location = new Point(navPage.automaticMessagingNav1.chooseGroupContacts1.label1.Location.X + 30, navPage.automaticMessagingNav1.chooseGroupContacts1.label1.Location.Y);
            }
            catch (Exception exc)
            {
                MaterialSnackBar SnackBarMessage = new MaterialSnackBar("Not Done 👎👎 -" + exc.Message, Strings.OK, true);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
             try
            {
                ...
                string result = MessageBox.Show("Please make sure that all WhatsBiz processes are closed before continuing!", "Alert", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning).ToString();
                if (result != "OK")
                {
                    return;
                }
                ...
                /* WebClient webClient = new WebClient();
                 string UpdatedFIlePath= webClient.DownloadString("https://pastebin.com/raw/3Erg7zEd");


                 string UpdateFIle = Config.GetTempFolderPath() + "\\" + Guid.NewGuid().ToString() + ".zip";
                 string extractPath = Config.WAPIFolderFolder();

                 webClient.DownloadFile(UpdatedFIlePath, UpdateFIle);
